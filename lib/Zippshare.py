# Recode => https://github.com/trollfist20/zippyshare-downloader
# Why did I recode it?
# because I want to make a few code changes to my style


from lib.utils import getStartandEndvalue
...
class Zippyshare:
	...
	def _download(self, urls, output: str):
		for url in urls:
			r = self._request_get(url)
			info = self._get_info(url, r)
			os.makedirs(output, exist_ok=True)
			info['name_file'] = os.path.join(output, info['name_file'])
			dl(info['download_url'], info['name_file'], progressbar=self._progress_bar, verbose=self._verbose, replace=self._replace)

	def download(self, *urls, output: str):
		self._download(urls, output)
